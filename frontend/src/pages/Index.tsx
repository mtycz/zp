    ...
    <Layout>
      ...
      <section className="py-16 border-y border-white/5">
        <div className="mx-auto max-w-7xl px-6">
          <FadeIn>
            <p className="text-center text-sm text-slate-500 mb-8">深受领先企业信赖</p>
          </FadeIn>
          <Stagger className="flex flex-wrap items-center justify-center gap-8 md:gap-16">
            {clientLogos.map((logo, i) => (
              <motion.div
                key={i}
                variants={fadeUp}
                className="text-xl font-semibold text-slate-500 hover:text-slate-300 transition-colors cursor-pointer"
              >
                {logo}
              </motion.div>
            ))}
