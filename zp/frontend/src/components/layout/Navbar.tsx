              ...
              <div className="flex flex-col gap-6 pt-8 px-4">
                {navigation.map((item) => (
                  <a
                    key={item.name}
                    href={item.href}
                    className="text-lg font-medium text-slate-300 transition-colors hover:text-white cursor-pointer text-center"
                ...
                <div className="mt-4 flex flex-col gap-3 px-2">
                  <Button variant="outline" className="w-full rounded-full border-white/20 text-white hover:bg-white/10" onClick={() => { setIsOpen(false); navigate('/login'); }}>
                    登录
                  </Button>
                  <Button
